...
int write_frame(int fd, char *header)
{   
    char dados[7] ={0x5D, 0x5c, 0x01, 0x03, 0x02, 0x5C};
    ...
    //strcat(dados,bcc2_str);
    dados[6] = bcc2;

    //envia dados + bcc2
    for(int j = 0; j < sizeof(dados); j++)
    {   
        //printf("%x ",dados[j]);
        if(dados[j] == 0x5C){
            char stuff_C[2] = {0x5D, 0x7C};  
            printf("%x ",stuff_C[0]);  printf("%x ",stuff_C[1]);  
            write(fd, stuff_C, 2);  
        }

        else if(dados[j] == 0x5D){ 
            char stuff_D[2] = {0x5D, 0x7D};
            printf("%x ",stuff_D[0]);  printf("%x ",stuff_D[1]);
            write(fd, stuff_D, 2);
        }
        else{
            write(fd,dados+j,1);
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    /*
    // SEND SET COMMAND
    if (send_command(fd, set) != 1)
        printf("ERROR SENDING SET COMMAND\n");
    else
...
*/
    ...
    char set_header[] = {FLAG, A, C_SET, BCC_SET};
    if (!write_frame(fd,set_header))
